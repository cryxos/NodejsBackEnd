...
export default {
    ...
    list: async (req, res, next) => {
        try {
            ...
            const reg = await models.Articulo.find({$or:[   {'nombre': new RegExp(valor,'i')}, {'description': new RegExp(valor,'i')}   ]} , {estado:0})
            .populate('categoria', {nombre:1})
            .sort({'createAt':-1});
    ...
    update:async (req, res, next) => {
        try {
            const reg = await models.Articulo.findByIdAndUpdate({_id:req.body._id}, {categoria:req.body.categoria, codigo:req.body.codigo,  nombre: req.body.nombre, description:req.body.description, precio_venta:req.body.precio_venta, stock:req.body.stock });
            res.status(200).json(reg);
